...
## v2.4.0 (2026-06-02) — 雾岛商店 & 购物车系统

### 新功能

- **雾岛世界商店**: 新增雾岛（mist_island）世界，含 83 项商品（武器、道具、防具、强化）
- **购物车系统**: 限定物品限购一次，非限定物品支持增减数量，确认兑换一次性提交
- **会话内退款**: 确认兑换前减少购物车数量 → 100% 返还花费
- **智能拥有检测**: 商店自动识别角色已拥有的物品（限定物品已拥有则不可再购）
- **角色绑定购物车**: 切换角色自动清空购物车，防止误操作

### 数据

- 雾岛物品数据来源：`无限团兑换列表（雾岛）.xlsx`（4 个工作表完整解析）
- 限定物品 62 项 / 非限定物品 21 项
- 武器 11 / 道具 56 / 防具 6 / 强化 10

### CI/CD

- **GitHub Actions 自动发布**: 推送 tag 自动构建 NSIS 安装包并发布到 GitHub Releases
